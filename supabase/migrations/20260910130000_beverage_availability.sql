-- Bebidas ativas independem de categoria e disponibilidade diária.
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    select p.id, p.name, p.public_name, p.product_type, p.price_cents, p.small_price_cents, p.large_price_cents
      ...
      where p.id=(v_item->>'product_id')::uuid and p.active and (p.product_type='beverage' or c.active) for share of p,c;
