...
Deno.serve(async request => {
  ...
  const enabled = Deno.env.get('WHATSAPP_INTEGRATION_ENABLED') === 'true';
  const verifyToken = Deno.env.get('WHATSAPP_VERIFY_TOKEN') ?? '';
  const appSecret = Deno.env.get('WHATSAPP_APP_SECRET') ?? '';
  const expectedPhoneNumberId = Deno.env.get('WHATSAPP_PHONE_NUMBER_ID') ?? '';

  if (request.method === 'GET') {
    if (!enabled) return json({ error: 'Integração oficial ainda não configurada' }, 503);
  ...
  if (parsed.inbound.some(message => message.phoneNumberId !== expectedPhoneNumberId)) return json({ error: 'Número comercial inesperado' }, 403);
