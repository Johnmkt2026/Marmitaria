...
function refreshMenu() { revalidatePath('/admin/cardapio'); revalidatePath('/cardapio'); }
...
export async function createCategory(input: unknown): Promise<MenuMutationResult> {
  ...
  try { const db = await requireAdminClient(); const { error } = await db.from('categories').insert({ name: parsed.data.name, sort_order: parsed.data.sortOrder }); if (error) return dbError(error.code); refreshMenu(); return { data: { message: 'Categoria criada.' } }; } catch (error) { return failure(error); }
}
...
export async function updateCategory(input: unknown): Promise<MenuMutationResult> {
  ...
  try { const db = await requireAdminClient(); const value = parsed.data; const { data, error } = await db.from('categories').update({ name: value.name, sort_order: value.sortOrder, active: value.active }).eq('id', value.id).eq('updated_at', value.updatedAt).select('id').maybeSingle(); if (error) return dbError(error.code); if (!data) return { error: 'Categoria alterada por outro administrador.', conflict: true }; refreshMenu(); return { data: { message: 'Categoria atualizada.' } }; } catch (error) { return failure(error); }
}
...
export async function createProduct(input: unknown): Promise<MenuMutationResult> {
  ...
  try { const db = await requireAdminClient(); const value = parsed.data; if (!await categoryExists(db, value.categoryId)) return { error: 'Categoria inexistente.' }; const { error } = await db.from('products').insert({ category_id: value.categoryId, name: value.name, description: value.description || null, price_cents: value.price, image_url: value.imageUrl || null, sort_order: value.sortOrder, active: value.active }); if (error) return dbError(error.code); refreshMenu(); return { data: { message: 'Produto criado.' } }; } catch (error) { return failure(error); }
}
export async function updateProduct(input: unknown): Promise<MenuMutationResult> {
  ...
  try { const db = await requireAdminClient(); const value = parsed.data; if (!await categoryExists(db, value.categoryId)) return { error: 'Categoria inexistente.' }; const { data, error } = await db.from('products').update({ category_id: value.categoryId, name: value.name, description: value.description || null, price_cents: value.price, image_url: value.imageUrl || null, sort_order: value.sortOrder, active: value.active }).eq('id', value.id).eq('updated_at', value.updatedAt).select('id').maybeSingle(); if (error) return dbError(error.code); if (!data) return { error: 'Produto alterado por outro administrador.', conflict: true }; refreshMenu(); return { data: { message: 'Produto atualizado.' } }; } catch (error) { return failure(error); }
}
...
export async function createOption(input: unknown): Promise<MenuMutationResult> {
  ...
  try { const db = await requireAdminClient(); const value = parsed.data; if (!await productExists(db, value.productId)) return { error: 'Produto inexistente.' }; const { error } = await db.from('product_options').insert({ product_id: value.productId, name: value.name, required: value.required, min_choices: value.minChoices, max_choices: value.maxChoices }); if (error) return dbError(error.code); refreshMenu(); return { data: { message: 'Opção criada.' } }; } catch (error) { return failure(error); }
}
export async function updateOption(input: unknown): Promise<MenuMutationResult> {
  ...
  try { const db = await requireAdminClient(); const value = parsed.data; const { data, error } = await db.from('product_options').update({ name: value.name, required: value.required, min_choices: value.minChoices, max_choices: value.maxChoices }).eq('id', value.id).eq('product_id', value.productId).eq('updated_at', value.updatedAt).select('id').maybeSingle(); if (error) return dbError(error.code); if (!data) return { error: 'Opção alterada por outro administrador.', conflict: true }; refreshMenu(); return { data: { message: 'Opção atualizada.' } }; } catch (error) { return failure(error); }
}
...
export async function updateAddon(input: unknown): Promise<MenuMutationResult> {
  ...
  try { const db = await requireAdminClient(); const value = parsed.data; if (!await optionBelongs(db, value.optionId, value.productId)) return { error: 'Opção não pertence ao produto.' }; const { data, error } = await db.from('product_addons').update({ option_id: value.optionId, name: value.name, price_cents: value.price, active: value.active, sort_order: value.sortOrder }).eq('id', value.id).eq('product_id', value.productId).eq('updated_at', value.updatedAt).select('id').maybeSingle(); if (error) return dbError(error.code); if (!data) return { error: 'Adicional alterado por outro administrador.', conflict: true }; refreshMenu(); return { data: { message: 'Adicional atualizado.' } }; } catch (error) { return failure(error); }
}
