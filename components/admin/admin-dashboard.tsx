...
export function AdminDashboard({ initial }: { initial: DashboardResult }) {
  ...
  return <>
    ...
    <div aria-busy={refreshing} className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{stats.map(([label, value], index) => <Card key={label} className={index === 1 ? 'border-terracotta-500/30 bg-terracotta-100/30' : ''}><p className="text-sm font-medium text-stone-500">{label}</p><div className="mt-2 text-3xl font-black tracking-tight text-brand-900">{value}</div></Card>)}</div>
