...
export function ReportsAnalytics({ initial }: { initial: ReportResult }) {
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <p className="mt-3 text-xs font-semibold text-stone-600">Período: {data.start_date.split('-').reverse().join('/')} a {data.end_date.split('-').reverse().join('/')} · America/Sao_Paulo</p>
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-2">
      <SectionCard eyebrow="Desempenho" title="Evolução diária" className="lg:col-span-2">
        <div className="mt-5 flex h-48 items-end gap-2 overflow-x-auto border-b border-stone-100 pb-1">{data.daily.map(day => <div key={day.date} title={`${day.date}: ${day.revenue_cents} centavos`} className="flex h-full min-w-9 flex-1 flex-col justify-end"><div className="mx-auto w-full max-w-12 rounded-t-lg bg-gradient-to-t from-terracotta-600 to-terracotta-400 transition-[height]" style={{ height: `${Math.max(day.revenue_cents ? 8 : 1, day.revenue_cents / maxRevenue * 100)}%` }}/><small className="mt-2 text-center text-[10px] font-bold text-stone-600">{shortDate(day.date)}</small></div>)}</div>
        <p className="mt-3 text-xs text-stone-600">Dias sem movimento permanecem na série com zero.</p>
      </SectionCard>
      ...
      <SectionCard eyebrow="Cardápio" title="Produtos mais vendidos">
        {data.products.length === 0 ? <Empty/> : <div className="mt-3 space-y-2">{data.products.map((product, index) => <article key={product.name} className="rounded-2xl border border-stone-200 bg-stone-50/70 p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><b className="block text-stone-950">{index + 1}. {product.name}</b><span className="text-xs font-bold text-terracotta-700">{product.quantity} unidades</span></div><b className="shrink-0 text-brand-900">{money(product.total_revenue_cents)}</b></div><p className="mt-2 text-xs text-stone-600">Produtos {money(product.product_revenue_cents)} + adicionais {money(product.addon_revenue_cents)}</p></article>)}</div>}
      </SectionCard>
    </div>
  </div>;
}
...
function Header({ period, choose, refreshing }: { period: Period; choose: (period: Period) => void; refreshing: boolean }) {
  return <header className="rounded-3xl bg-gradient-to-br from-brand-900 to-brand-700 p-5 text-white shadow-[0_18px_50px_rgba(24,74,55,0.16)] sm:p-7"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-white/85">Inteligência da operação</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Relatórios</h1><p className="mt-2 max-w-xl text-sm font-medium text-white/90">Acompanhe vendas, clientes e desempenho do cardápio em um só lugar.</p></div><label className="text-xs font-bold uppercase tracking-wider text-white">Período<select aria-label="Período do relatório" value={period} disabled={refreshing} onChange={event => choose(event.target.value as Period)} className="mt-2 block min-h-12 w-full rounded-xl border border-stone-300 bg-white px-4 font-bold normal-case tracking-normal text-stone-950 shadow-sm outline-none focus-visible:border-brand-900 focus-visible:ring-2 focus-visible:ring-white disabled:cursor-wait disabled:text-stone-600 sm:w-56"><option value="today">Hoje</option><option value="7">Últimos 7 dias</option><option value="30">Últimos 30 dias</option><option value="custom">Período personalizado</option></select></label></div></header>;
}

function Kpi({ label, value, featured = false, accent = false }: { label: string; value: ReactNode; featured?: boolean; accent?: boolean }) {
  return <section className={`min-w-0 rounded-3xl border border-stone-200 p-4 shadow-[0_10px_35px_rgba(41,55,38,0.06)] ${featured ? 'bg-brand-900 text-white' : 'bg-white text-stone-950'} ${accent ? 'ring-1 ring-emerald-300' : ''}`}><p className={`text-xs font-black uppercase tracking-wider ${featured ? 'text-white' : 'text-stone-700'}`}>{label}</p><div className={`mt-2 break-words text-xl font-black sm:text-2xl ${featured ? 'text-white' : accent ? 'text-emerald-800' : 'text-stone-950'}`}>{value}</div></section>;
}
...
function MiniMetric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl bg-stone-50 p-3"><span className="text-xs font-bold text-stone-700">{label}</span><strong className="mt-1 block text-xl text-brand-950">{value}</strong></div>;
}

function Line({ label, detail, cents }: { label: string; detail: string; cents: number }) {
  return <div className="mt-3 flex items-center justify-between gap-3 rounded-2xl border border-stone-200 bg-stone-50/70 p-3"><span><b className="text-stone-950">{label}</b><small className="mt-0.5 block text-stone-600">{detail}</small></span><b className="shrink-0 text-brand-900">{money(cents)}</b></div>;
}

function Empty() {
  return <p className="mt-4 rounded-2xl bg-stone-50 p-4 text-sm text-stone-600">Sem movimento no período.</p>;
}
