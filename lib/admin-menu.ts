...
export async function getAdminMenu(): Promise<AdminMenuData> {
  ...
  const [categories, products, availability, options, addons] = await Promise.all([
    ...
    db.from('products').select('id,category_id,name,description,price_cents,image_url,sort_order,active,updated_at').order('sort_order').order('name').returns<ProductRow[]>(),
