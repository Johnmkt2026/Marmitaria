...
import type { WhatsAppCenterData, WhatsAppCenterResult, WhatsAppConversation, WhatsAppMessage, WhatsAppMessagesResult, WhatsAppOrder } from '@/lib/admin-whatsapp-types';
...
export async function getWhatsAppCenterData(): Promise<WhatsAppCenterData> {
  ...
  const [conversationsResult, ordersResult, settingsResult, whatsappSettingsResult] = await Promise.all([
    ...
    db.from('whatsapp_settings').select('automations_enabled').eq('singleton',true).single<{ automations_enabled: boolean }>(),
  ]);
  ...
  return { conversations: conversationsResult.data, fallback_orders: ordersResult.data,
    integration_enabled: process.env.WHATSAPP_INTEGRATION_ENABLED === 'true', automations_enabled: whatsappSettingsResult.data.automations_enabled,
...
export async function getWhatsAppMessages(conversationId: string, beforeCreatedAt?: string, beforeId?: string): Promise<WhatsAppMessage[]> {
  const db = await requireAdminClient();
  let query = db.from('whatsapp_messages').select('id,conversation_id,external_message_id,direction,message_type,content_text,template_name,status,automatic,received_at,accepted_at,sent_at,delivered_at,read_at,failed_at,failure_message,created_at')
